<?php namespace App\Http\Controllers;
	...
	class AdminDataFarmMitraController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Silahkan Scan Untuk Mengetahui Detail","name"=>"kode","callback"=>function($row){
			    $link=url('detail_ternak_mitra/'.$row->kode);
...
// 			$this->col[] = ["label"=>"Betina","name"=>"id_betina","join"=>"data_farm,kode"];
			$this->col[] = ["label"=>"Pemilik Ternak","name"=>"id_users","join"=>"cms_users,name"];
    ...
    $this->form[] = ['label'=>'Jenis Ternak','name'=>'jenis','type'=>'select2','width'=>'col-sm-10','datatable'=>'jenis_kambing,jenis'];
			...
			$this->form[] = ['label'=>'Status','name'=>'posisi','type'=>'select','validation'=>'required|string|min:1|max:5000','width'=>'col-sm-10','dataenum'=>'kandang;terjual;mati'];
					...
					$this->form[] = ['label'=>'Indukan Jantan','name'=>'id_jantan','type'=>'select2','width'=>'col-sm-10','datatable'=>'data_farm_mitra,kode','datatable_where'=>'id_users = '.CRUDBooster::myId()];
			
			$this->form[] = ['label'=>'Indukan Betina','name'=>'id_betina','type'=>'select2','width'=>'col-sm-10','datatable'=>'data_farm_mitra,kode','datatable_where'=>'id_users ='.CRUDBooster::myId()];
...
// 			$this->form[] = ['label'=>'Pemilik ternak','name'=>'id_users','type'=>'select2','width'=>'col-sm-10','datatable'=>'cms_users,name'];
	        ...
	        */
	        $this->index_statistic = array();
	         ...
	         $this->index_statistic[] = ['label'=>'Jumlah Ternak di Mitra','count'=>DB::table('data_farm_mitra')
	          ->where('id_users',$id_datafarm_admin)
	         ->count(),'icon'=>'fa fa-check','color'=>'success'];
	         
	         $this->index_statistic[] = ['label'=>'Jumlah Ternak di Mitra Jantan','count'=>DB::table('data_farm_mitra')->where('jenis_kelamin','jantan')
	          ->where('id_users',$id_datafarm_admin)
	         ->count(),'icon'=>'fa fa-check','color'=>'success'];
	         
	         $this->index_statistic[] = ['label'=>'Jumlah Ternak di Mitra Betina','count'=>DB::table('data_farm_mitra')->where('jenis_kelamin','betina')
	          ->where('id_users',$id_datafarm_admin)
	         ->count(),'icon'=>'fa fa-check','color'=>'success'];
  ...
  Ini adalah menu mitra atau karyawan yang bisa mengelolanya, silahkan login sebgai mitra untuk mengelola halaman ini. <br> Silahkan isi data ternak anda. 
</div>';
	        ...
	        */
	        ...
	        $this->sub_module[] = ['label'=>'Jadwal','path'=>'jadwal_farm_mitra','parent_columns'=>'kode,nama','foreign_key'=>'id_kambing','button_color'=>'success','button_icon'=>'fa fa-bars'];
	        
	        $this->sub_module[] = ['label'=>'ADg Ternak','path'=>'adg_ternak_mitra','parent_columns'=>'kode,nama','foreign_key'=>'id_kambing','button_color'=>'danger','button_icon'=>'fa fa-bars'];
	        
	          $this->sub_module[] = ['label'=>'Catatan Kesehatan','path'=>'kesehatan_ternak_mitra','parent_columns'=>'kode,nama','foreign_key'=>'id_kambing','button_color'=>'warning','button_icon'=>'fa fa-bars'];
	        
	        $this->sub_module[] = ['label'=>'Susu Ternak','path'=>'susu_ternak_mitra','parent_columns'=>'kode,nama','foreign_key'=>'id_ternak','button_color'=>'warning','button_icon'=>'fa fa-bars'];
	    ...
	    public function hook_query_index(&$query) {
	        ...
	        $id=CRUDBooster::myId();
	          if(CRUDBooster::myPrivilegeName() == "mitra"){
                ...
                $query->where('data_farm_mitra.id_users',$id);
                ...
                $query->whereIn('data_farm_mitra.id_users',$data);
              }
	    ...
	    */
	    public function hook_after_add($id) {        
	        //Your code here
	        $cek=DB::table('data_farm_mitra')->where('id',$id)->first();
	        ...
	        $date=date('Ymd');
	        $data['kode']="M".$gender.$date.$id;
	        DB::table('data_farm_mitra')->where('id',$id)->update($data);
	    ...
	    */
	    public function hook_after_delete($id) {
	     ...
	     public function getKandang($id){
	    	...
	    	$cek=DB::table('data_farm_mitra')->where('id',$id)->update($status);
...
public function getPdfexport($id){
		 	...
		 	$row=DB::table('data_farm_mitra')->where('id',$id)->get();
			  $pdf = PDF::loadView('export_mitra',compact('row'))
			  ->setPaper('a4','potret');
