<?php namespace App\Http\Controllers;
	...
	class AdminPengeluaranFarmController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Nota / Bukti TF","name"=>"foto","image"=>true];
	
			# END COLUMNS DO NOT REMOVE THIS LINE

			# START FORM DO NOT REMOVE THIS LINE
			$this->form = [];
			$this->form[] = ['label'=>'Keperluan','name'=>'keperluan','type'=>'text','validation'=>'required|string|min:5|max:5000','width'=>'col-sm-10'];
				$this->form[] = ['label'=>'Jenis Keperluan','name'=>'jenis_keperluan','type'=>'select','validation'=>'required|string','width'=>'col-sm-10','dataenum'=>'pakan ternak;asset peternakan;hewan ternak;obat ternak;Gaji Karyawan;operasional'];
			$this->form[] = ['label'=>'Harga','name'=>'harga','type'=>'money','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Tgl','name'=>'tgl','type'=>'date','validation'=>'required|date','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Nota / Bukti TF','name'=>'foto','type'=>'upload','validation'=>'image|max:3000','width'=>'col-sm-10','help'=>'File types support : JPG, JPEG, PNG, GIF, BMP'];
			
			
			if(CRUDBooster::myPrivilegeName() != "Super Administrator"){}else{
	        ...
	        */
	        ...
	        $this->index_statistic[] = ['label'=>'Pengeluaran Pakan Ternak','count'=>'Rp.'.number_format(DB::table('pengeluaran_farm')->where('jenis_keperluan','pakan ternak')
	         ->where('id_users',$id_pengeluaran_admin)
	        ->sum('harga')),'icon'=>'fa fa-check','color'=>'info'];
	        
	        $this->index_statistic[] = ['label'=>'Pengeluaran Beli Hewan','count'=>'Rp.'.number_format(DB::table('pengeluaran_farm')
	        ->where('jenis_keperluan','hewan ternak')
	        ->where('id_users',$id_pengeluaran_admin)->sum('harga')),'icon'=>'fa fa-check','color'=>'info'];
	        
	        $this->index_statistic[] = ['label'=>'Pengeluaran Obat Ternak','count'=>'Rp.'.number_format(DB::table('pengeluaran_farm')->where('jenis_keperluan','obat ternak')
	         ->where('id_users',$id_pengeluaran_admin)
	        ->sum('harga')),'icon'=>'fa fa-check','color'=>'info'];
	        
	         $this->index_statistic[] = ['label'=>'Pengeluaran Gaji Karyawan','count'=>'Rp.'.number_format(DB::table('pengeluaran_farm')->where('jenis_keperluan','Gaji Karyawan')
	          ->where('id_users',$id_pengeluaran_admin)
	         ->sum('harga')),'icon'=>'fa fa-check','color'=>'info'];
	         
	         $this->index_statistic[] = ['label'=>'Pengeluaran Asset Peternakan','count'=>'Rp.'.number_format(DB::table('pengeluaran_farm')->where('jenis_keperluan','asset peternakan')
	          ->where('id_users',$id_pengeluaran_admin)
	         ->sum('harga')),'icon'=>'fa fa-check','color'=>'info'];
	         
	         $this->index_statistic[] = ['label'=>'Operasional Peternakan','count'=>'Rp.'.number_format(DB::table('pengeluaran_farm')->where('jenis_keperluan','operasional')
	          ->where('id_users',$id_pengeluaran_admin)
	         ->sum('harga')),'icon'=>'fa fa-check','color'=>'info'];
  ...
  $this->pre_index_html ='<div class="modal fade" id="myModal" role="dialog">
	    ...
	    */
	    public function hook_before_delete($id) {
	        //Your code here
	         $gambar =DB::table('pengeluaran_farm')->where('id',$id)->first();
	        if($gambar->foto!=null){
	    ...
	    */
	    public function hook_after_delete($id) {
	      ...
	      public function getPdfexport($id){
		 	...
		 	$row=DB::table('pengeluaran_farm')->where('id',$id)->get();
