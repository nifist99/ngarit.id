<?php namespace App\Http\Controllers;
	...
	class AdminDataFarmController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Silahkan Scan Untuk Mengetahui Detail","name"=>"kode","callback"=>function($row){
			    ...
			    ."<p>silahkan download qrcode di detail ternak</p>
			    ";
			}];
...
// 			$this->form[] = ['label'=>'Pemilik ternak','name'=>'id_users','type'=>'select2','width'=>'col-sm-10','datatable'=>'cms_users,name'];
	        ...
	        */
	        ...
	        $this->index_button []= ['label'=>'Tambah Data Sapi',
	        'icon'=>'fa fa-plus','url'=>CRUDBooster::mainpath('addsapi')];
	       ...
	       if(CRUDBooster::myPrivilegeName()=="mitra"){
	        ...
	        $this->index_statistic = array();
	         $this->index_statistic[] = ['label'=>'Jumlah Ternak','count'=>DB::table('data_farm')
	   ...
	   $(document).ready(function(){
        ...
        $('#btn_add_new_data').html('Tambah Data Kambing');
  ...
  $data['jenis_kambing']=DB::table('jenis_kambing')->where('kategori','sapi')->get();
  
  $result_sapi=[];
  foreach($data['jenis_kambing'] as $key){
      $list=$key->id;
      array_push($result_sapi,$list);
  }
  ...
  ->whereIn('jenis_kambing.id',$result_sapi)
  ->select('data_farm.*')
  ->get();
...
//   ->get();
  ...
  return $this->view('admin.add_datafarm',$data);
}
 ...
 public function getAdd() {
  ...
  $data = [];
  $data['page_title'] = 'Add Data Kambing';
  $data['jenis_kambing']=DB::table('jenis_kambing')->where('kategori','kambing')->get();
...
//   oke
  ...
  return $this->view('admin.add_datafarm',$data);
	    ...
	    public function postSimpan() {        
	       ...
	       $id=DB::table('data_farm')->insertGetId($data);
	       $cek=DB::table('data_farm')->where('id',$id)->first();
	        if($cek->jenis_kelamin=="jantan"){
	            $gender="J";
	        }else{
