<?php namespace App\Http\Controllers;
	...
	class AdminArtikelBlogController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Kategori","name"=>"id_kategori","join"=>"kategori,id"];
			# END COLUMNS DO NOT REMOVE THIS LINE

			# START FORM DO NOT REMOVE THIS LINE
			$this->form = [];
			$this->form[] = ['label'=>'Judul','name'=>'judul','type'=>'textarea','validation'=>'required|string|min:5|max:5000','width'=>'col-sm-10'];
			...
			$this->form[] = ['label'=>'Kategori','name'=>'id_kategori','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'kategori,id'];
  ...
  //First, Add an auth
   ...
   $data = [];
   $data['page_title'] = 'Artikel Ternak';
   $data['result'] = DB::table('artikel')->where('artikel.status','publish')
   ->join('cms_users','artikel.id_users','=','cms_users.id')
   ->select('cms_users.name','cms_users.photo','artikel.*')
   ->get();
    
   //Create a view. Please use `view` method instead of view method from laravel.
   return $this->view('admin.admin',$data);
}
...
public function getDetail($id) {
  ...
  $data['page_title'] = 'Artikel';
   $data['row'] = DB::table('artikel')->where('artikel.id',$id)
   ->join('cms_users','artikel.id_users','=','cms_users.id')
   ->select('cms_users.name','cms_users.photo','artikel.*')
   ->first();
   
   $data['komen']=DB::table('komen_artikel')
   ->where('komen_artikel.id_artikel',$id)
   ->join('cms_users','komen_artikel.id_users','=','cms_users.id')
   ->select('komen_artikel.*','cms_users.*')
   ->get();
   
   $data['jumlah_komen']=DB::table('komen_artikel')
   ->where('id_artikel',$id)->count();
   $data['jumlah_like']=DB::table('like_artikel')
   ->where('id_artikel',$id)->count();
   
   $data['cek_like']=DB::table('like_artikel')
   ->where('id_artikel',$id)
   ->where('id_users',CRUDBooster::myId())
   ->first();
  
  //Please use view method instead view method from laravel
  return $this->view('admin.detail_artikel',$data);
}
	      ...
	      public function getKomen() {
	        ...
	        $data['id_artikel']=g('id_artikel');
	        $data['created_at']=date('Y-m-d');
	        
	            if (DB::table('komen_artikel')->insert($data)) {				
		    $res = redirect()->back()->with(["message"=>"Succesfully Add Comment",'message_type'=>'success'])->withInput();
		    }else{
		    $res = redirect()->back()->with(["message"=>"Error add comment",'message_type'=>'warning'])->withInput();
		    	}
		    		\Session::driver()->save();
		    	$res->send();
		    	exit();
	        ...
	        $data['id_artikel']=g('id_artikel');
	            ...
	            if (DB::table('like_artikel')->insert($data)) {				
		    $res = redirect()->back()->with(["message"=>"Succesfully Like Artikel",'message_type'=>'success'])->withInput();
		    }else{
		    $res = redirect()->back()->with(["message"=>"Error Like artikel",'message_type'=>'warning'])->withInput();
		    	}
		    		\Session::driver()->save();
		    	$res->send();
		    	exit();
