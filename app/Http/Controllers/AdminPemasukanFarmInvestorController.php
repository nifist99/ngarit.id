<?php namespace App\Http\Controllers;
	...
	class AdminPemasukanFarmInvestorController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "pemasukan_farm_investor";
			...
			$this->form[] = ['label'=>'Pemasukan','name'=>'pemasukan','type'=>'textarea','validation'=>'required|string|min:5|max:5000','width'=>'col-sm-10'];
				$this->form[] = ['label'=>'Jenis Pemasukan','name'=>'jenis_pemasukan','type'=>'select','validation'=>'required|string','width'=>'col-sm-10','dataenum'=>'jual asset;jual hewan ternak'];
...
// 			$this->form[] = ['label'=>'Users','name'=>'id_users','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'users,name'];
	        ...
	        */
	        $this->index_statistic = array();
	         ...
	         $this->index_statistic[] = ['label'=>'Pemasukan Jual Asset','count'=>'Rp.'.number_format(DB::table('pemasukan_farm_investor')->where('jenis_pemasukan','jual asset')
	          ->where('id_users',$id_pemasukan_admin)
	         ->sum('harga')),'icon'=>'fa fa-check','color'=>'success'];
	         
	         $this->index_statistic[] = ['label'=>'Pemasukan Jual Ternak','count'=>'Rp.'.number_format(DB::table('pemasukan_farm_investor')->where('jenis_pemasukan','jual hewan ternak')
	          ->where('id_users',$id_pemasukan_admin)
	         ->sum('harga')),'icon'=>'fa fa-check','color'=>'success'];
  ...
  <b>Silahkan isi pemasukan keuangan investor, menu ini hanya investor yang bisa mengelolanya</b>.
</div>';
 $this->pre_index_html = '<div class="alert alert-info" role="alert">
 ...
 $this->pre_index_html ='<div class="modal fade" id="myModal" role="dialog">
	    ...
	    */
	    public function hook_before_delete($id) {
	        //Your code here
	          $gambar =DB::table('pemasukan_farm_investor')->where('id',$id)->first();
	        if($gambar->foto!=null){
	    ...
	    */
	    ...
	    public function getPdfexport($id){
		 	...
		 	$row=DB::table('pemasukan_farm_investor')->where('id',$id)->get();
	...
	if(!isset($start)){
	    if(!isset($status)){
	     $row=DB::table('pemasukan_farm_investor')
		->where('id_users',$id)
		->get();
	    }else{
		$row=DB::table('pemasukan_farm_investor')
