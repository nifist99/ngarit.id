<?php namespace App\Http\Controllers;
	...
	class AdminPemasukanFarmController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form[] = ['label'=>'Pemasukan','name'=>'pemasukan','type'=>'textarea','validation'=>'required|string|min:5|max:5000','width'=>'col-sm-10'];
				$this->form[] = ['label'=>'Jenis Pemasukan','name'=>'jenis_pemasukan','type'=>'select','validation'=>'required|string','width'=>'col-sm-10','dataenum'=>'jual pakan ternak;jual susu ternak;jual hewan ternak;jual kotoran'];
	       ...
	       if($_GET['parent_id']==null){
	        ...
	        $this->index_statistic = array();
	         $this->index_statistic[] = ['label'=>'Pemasukan Pakan Ternak','count'=>'Rp.'.number_format(DB::table('pemasukan_farm')->where('jenis_pemasukan','pakan ternak')
	          ->where('id_users',$id_pemasukan_admin)
	         ->sum('harga')),'icon'=>'fa fa-check','color'=>'success'];
	         
	         $this->index_statistic[] = ['label'=>'Pemasukan Jual Susu','count'=>'Rp.'.number_format(DB::table('pemasukan_farm')->where('jenis_pemasukan','jual susu ternak')
	          ->where('id_users',$id_pemasukan_admin)
	         ->sum('harga')),'icon'=>'fa fa-check','color'=>'success'];
	         
	         $this->index_statistic[] = ['label'=>'Pemasukan Jual Ternak','count'=>'Rp.'.number_format(DB::table('pemasukan_farm')->where('jenis_pemasukan','jual hewan ternak')
	          ->where('id_users',$id_pemasukan_admin)
	         ->sum('harga')),'icon'=>'fa fa-check','color'=>'success'];
	         
	          $this->index_statistic[] = ['label'=>'Pemasukan Jual Kotoran Ternak','count'=>'Rp.'.number_format(DB::table('pemasukan_farm')->where('jenis_pemasukan','jual kotoran')
	          ->where('id_users',$id_pemasukan_admin)
	         ->sum('harga')),'icon'=>'fa fa-check','color'=>'success'];
  ...
  Silahkan isi data pemasukan keuangan peternakan anda disini, agar tercatat rapi.
</div>';
 ...
 $this->pre_index_html ='<div class="modal fade" id="myModal" role="dialog">
							    <div class="modal-dialog">
							      <div class="modal-content">
							        ...
							        <label>Nama File</label>
							         ...
							         <option value="jual pakan ternak">jual pakan ternak</option>
							         <option value="jual kotoran">jual kotoran</option>
							         <option value="jual hewan ternak">hjual ewan ternak</option>
							         <option value="jual susu ternak">jual susu ternak</option>
							         </select>
							         </div>
	    ...
	    */
	    public function hook_before_delete($id) {
	        //Your code here
	         $gambar =DB::table('pemasukan_farm')->where('id',$id)->first();
	        if($gambar->foto!=null){
	    ...
	    */
	    public function hook_after_delete($id) {
	        //Your code here
		 	...
		 	$row=DB::table('pemasukan_farm')->where('id',$id)->get();
			  $pdf = PDF::loadView('laporan_pemasukan',compact('row'))
			  ->setPaper('a4','potret');
			  return $pdf->stream('Laporan Pemasukan'.$row[0]->jenis_pemasukan.'.pdf');
	...
	if(!isset($start)){
	    if(!isset($status)){
	     $row=DB::table('pemasukan_farm')
		->where('id_users',$id)
		->get();
	    }else{
		$row=DB::table('pemasukan_farm')
		->where('jenis_pemasukan',$status)
		->where('id_users',$id)
		->get();
	    }
		...
		$pdf = PDF::loadView('laporan_pemasukan_all',compact('data'))
			  ->setPaper('a4','potret');
