<?php namespace App\Http\Controllers;
	...
	class AdminChatToAdminController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form[] = ['label'=>'Admin','name'=>'id_admin','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'admin,id'];
			...
			$this->form[] = ['label'=>'Chat By','name'=>'chat_by','type'=>'textarea','validation'=>'required|string|min:5|max:5000','width'=>'col-sm-10'];
			...
			//$this->form[] = ["label"=>"Chat By","name"=>"chat_by","type"=>"textarea","required"=>TRUE,"validation"=>"required|string|min:5|max:5000"];
			# OLD END FORM
   ...
   $data = [];
   $id=CRUDBooster::myId();
   $data['page_title'] = 'Konsultasi Chat Ke Admin';
   $role=DB::table('cms_privileges')->where('name','admin')->first();
   
   $key = DB::table('cms_users')
 ...
 //   send notif web ke admin
// $config['content']="pesan dari peternak";
// $config['to']=$key->id;
...
// CRUDBooster::sendNotification($config);
   ...
   return $this->view('admin.chat_users',$data);
  }
