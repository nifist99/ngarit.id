<?php namespace App\Http\Controllers;
	...
	class AdminPengeluaranFarmMitraController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "pengeluaran_farm_mitra";
			# END CONFIGURATION DO NOT REMOVE THIS LINE

			# START COLUMNS DO NOT REMOVE THIS LINE
			$this->col = [];
			$this->col[] = ["label"=>"Keperluan","name"=>"keperluan"];
			$this->col[] = ["label"=>"Jenis Kperluan","name"=>"jenis_keperluan"];
...
// 			$this->form[] = ['label'=>'Users','name'=>'id_users','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'users,name'];
	        ...
	        */
	        ...
	        $this->index_statistic = array();
	        $this->index_statistic[] = ['label'=>'Pengeluaran Pakan Ternak','count'=>'Rp.'.number_format(DB::table('pengeluaran_farm_mitra')->where('jenis_keperluan','pakan ternak')
	         ->where('id_users',$id_pengeluaran_admin)
	        ->sum('harga')),'icon'=>'fa fa-check','color'=>'info'];
	        
	        $this->index_statistic[] = ['label'=>'Pengeluaran Beli Hewan','count'=>'Rp.'.number_format(DB::table('pengeluaran_farm_mitra')
	        ->where('jenis_keperluan','hewan ternak')
	        ->where('id_users',$id_pengeluaran_admin)->sum('harga')),'icon'=>'fa fa-check','color'=>'info'];
	        
	        $this->index_statistic[] = ['label'=>'Pengeluaran Obat Ternak','count'=>'Rp.'.number_format(DB::table('pengeluaran_farm_mitra')->where('jenis_keperluan','obat ternak')
	         ->where('id_users',$id_pengeluaran_admin)
	        ->sum('harga')),'icon'=>'fa fa-check','color'=>'info'];
	        
	         $this->index_statistic[] = ['label'=>'Pengeluaran Asset Peternakan','count'=>'Rp.'.number_format(DB::table('pengeluaran_farm_mitra')->where('jenis_keperluan','asset peternakan')
	          ->where('id_users',$id_pengeluaran_admin)
	         ->sum('harga')),'icon'=>'fa fa-check','color'=>'info'];
	         
	         $this->index_statistic[] = ['label'=>'Operasional Peternakan','count'=>'Rp.'.number_format(DB::table('pengeluaran_farm_mitra')->where('jenis_keperluan','operasional')
	          ->where('id_users',$id_pengeluaran_admin)
	         ->sum('harga')),'icon'=>'fa fa-check','color'=>'info'];
  ...
  Ini adalah menu mitra atau karyawan yang bisa mengelolanya, silahkan login sebgai mitra untuk mengelola halaman ini. <br> Silahkan isi pengeluaran keuangan peternakan anda dimenu ini, agar tercatat rapi. 
</div>';
	    ...
	    */
	    public function hook_after_add($id) {        
	        //Your code here
	        $cek=DB::table('pengeluaran_farm_mitra')->where('id',$id)->first();
	        if($cek->jenis_keperluan=="asset"){
	            $data['nama']=$cek->keperluan;
	            $data['harga']=$cek->harga;
	            $data['foto']=$cek->foto;
	            $data['created_by']=$cek->id_users;
	            $data['tgl']=$cek->tgl;
	            $ok=DB::table('asset_farm_mitra')->insert($data);
	        }
