<?php namespace App\Http\Controllers;
	...
	class AdminDataFarmMitra40Controller extends \crocodicstudio\crudbooster\controllers\CBController {
  ...
  //First, Add an auth
   ...
   $data['cek_role_login']=CRUDBooster::myPrivilegeName();
   $id=CRUDBooster::myId();
              if(CRUDBooster::myPrivilegeName()=="mitra"){
             $data['total_pengeluaran'] = DB::table('pengeluaran_farm_mitra')
             ->where('id_users',$id)->sum('harga');
             
        $data['pemasukan_farm'] = DB::table('pemasukan_farm_mitra')
        ->where('id_users',$id)->sum('harga');
        
        $data['data'] = DB::table('data_farm_mitra')->where('id_users',$id)->count();
            
            $data['ternak_mati']=DB::table('data_farm_mitra')->where('id_users',$id)->where('posisi','mati')->count();
            
            $data['kandang']=DB::table('data_farm_mitra')->where('id_users',$id)->where('posisi','kandang')->count();
            
            $data['sold_out']=DB::table('data_farm_mitra')->where('id_users',$id)->where('posisi','terjual')->count();
            
            $data['jantan']=DB::table('data_farm_mitra')->where('id_users',$id)->where('jenis_kelamin','jantan') ->where('posisi','!=','mati')->count();
            
            $data['betina']=DB::table('data_farm_mitra')->where('id_users',$id)->where('jenis_kelamin','betina') ->where('posisi','!=','mati')->count();
            
            // data farm peternak
            // $cekmitra=DB::table('data_mitra')->where('id_login',$id)->first();
            $data['jumlah_ternak_peternak'] = DB::table('data_farm')->where('id_mitra',$id)->count();
            
            $data['jumlah_ternak_jantan_peternak'] = DB::table('data_farm')->where('id_mitra',$id)
            ->where('jenis_kelamin','jantan')
            ->where('posisi','!=','mati')
            ->count();
            
            $data['jumlah_ternak_betina_peternak'] = DB::table('data_farm')->where('id_mitra',$id)
            ->where('jenis_kelamin','betina')
            ->where('posisi','!=','mati')
            ->count();
            
            $data['jumlah_ternak_mati_peternak'] = DB::table('data_farm')
            ->where('id_mitra',$id)
            ->where('posisi','mati')
            ->count();
            ...
            $data['jumlah_ternak_investor'] = DB::table('data_farm_investor')->where('id_mitra',$id)->count();
            
            $data['jumlah_ternak_jantan_investor'] = DB::table('data_farm_investor')->where('id_mitra',$id)
            ->where('jenis_kelamin','jantan') ->where('posisi','!=','mati')
            ->count();
            
            $data['jumlah_ternak_betina_investor'] = DB::table('data_farm_investor')->where('id_mitra',$id)
            ->where('jenis_kelamin','betina') ->where('posisi','!=','mati')
            ->count();
            
            $data['jumlah_ternak_mati_investor'] = DB::table('data_farm_investor')->where('id_mitra',$id)
            ->where('posisi','mati')
            ->count();
                  ...
                  $data['total_pengeluaran'] = DB::table('pengeluaran_farm_mitra')
             ->whereIn('id_users',$data)->sum('harga');
             
        $data['pemasukan_farm'] = DB::table('pemasukan_farm_mitra')
        ->whereIn('id_users',$data)->sum('harga');
        
        $data['data'] = DB::table('data_farm_mitra')
        ->whereIn('id_users',$data)->count();
            
            $data['ternak_mati']=DB::table('data_farm_mitra')
            ->whereIn('id_users',$data)->where('posisi','mati')->count();
            
            $data['kandang']=DB::table('data_farm_mitra')
            ->whereIn('id_users',$data)->where('posisi','kandang')->count();
            
            $data['sold_out']=DB::table('data_farm_mitra')
            ->whereIn('id_users',$data)->where('posisi','terjual')->count();
            
            $data['jantan']=DB::table('data_farm_mitra')
            ->wherein('id_users',$data)->where('jenis_kelamin','jantan') ->where('posisi','!=','mati')->count();
            
            $data['betina']=DB::table('data_farm_mitra')
            ->whereIn('id_users',$data)->where('jenis_kelamin','betina') ->where('posisi','!=','mati')->count();
              }
        ...
        $data['data'] = DB::table('data_farm_mitra')->count();
            
            $data['ternak_mati']=DB::table('data_farm_mitra')->where('posisi','mati')->count();
            
            $data['kandang']=DB::table('data_farm_mitra')->where('posisi','kandang')->count();
            
            $data['sold_out']=DB::table('data_farm_mitra')->where('posisi','terjual')->count();
            
            $data['jantan']=DB::table('data_farm_mitra')
            ->where('jenis_kelamin','jantan') ->where('posisi','!=','mati')->count();
            
            $data['betina']=DB::table('data_farm_mitra')
            ->where('jenis_kelamin','betina')->where('posisi','!=','mati')->count();
              }
    
   return $this->view('dashboard_mitra',$data);
}
